...
tickers <- c("AXP", "AMZN", "AMP", "GOOG", "MSFT", "AAPL")

#Calculate Returns: Daily
portfolioPrices <- NULL
for (Ticker in tickers)
  portfolioPrices <- cbind(portfolioPrices,
                           getSymbols.google(Ticker, from="2017-01-01", auto.assign=FALSE)[,4])
...
Stock_Data <- tickers %>% lapply(function(x) getSymbols.google(x, from="2017-01-01", auto.assign=FALSE)[,4]) %>%
...
#############################################################################################
###########################Step THREE: Examine Constraints and Stats#########################
#############################################################################################
